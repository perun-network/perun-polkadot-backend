...
func DecodeError(_meta *types.Metadata, err types.DispatchError) error {
	if !_meta.IsMetadataV13 {
		return errors.Wrap(ErrUnknownError, "wrong meta data version")
	}
	if !err.HasModule {
		return errors.Wrap(ErrUnknownError, "no module in error")
	}
	meta := _meta.AsMetadataV13
	if int(err.Module) >= len(meta.Modules) {
		return errors.Wrap(ErrUnknownError, "module index out of range")
	}
	module := meta.Modules[err.Module]
